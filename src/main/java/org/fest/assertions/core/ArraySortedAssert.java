 ...
 * the differences being that we can't check that - for empty List - the list parameter is comparable or compatible with
 * given comparator due to type erasure.
 *
 * @param <S> the "self" type of this assertion class that must be a array type (e.g. arrays, collections).<br>
 *          Please read &quot;<a href="http://bit.ly/anMa4g" target="_blank">Emulating 'self types' using Java Generics
 ...
 * @author Yvonne Wang
 */
public interface ArraySortedAssert<S> {

  /**
   * Verifies that the actual array is sorted into ascending order according to the natural ordering of its elements.
   * <p>
   * All array elements must be primitive or implement the {@link Comparable} interface and must be mutually comparable
   ...
   * <li>a array composed of {True, "abc", False} is <b>NOT ok</b> because elements are not mutually comparable (even
   * though each element type implements Comparable)</li>
   * </ul>
   * Empty or one element arrays are considered sorted (unless the array element type is not Comparable).</br></br>
   *
   * @return {@code this} assertion object.
   *
   * @throws AssertionError if the actual array is not sorted into ascending order according to the natural ordering of
   *           its elements.
