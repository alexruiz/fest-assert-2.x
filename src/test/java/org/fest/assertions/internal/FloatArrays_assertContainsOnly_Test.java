...
/**
 * Tests for <code>{@link FloatArrays#assertContainsOnly(Description, float[], float[])}</code>.
 *
 * @author Alex Ruiz
 * @author Yvonne Wang
 */
public class FloatArrays_assertContainsOnly_Test {
  ...
  public void should_fail_if_actual_does_not_contain_only_other() {
    ...
    Set<Float> notExpected = asSet(actual);
    Set<Float> notFound = containsOnly(notExpected, other);
    try {
      arrays.assertContainsOnly(description, actual, other);
  ...
  public void should_pass_if_actual_contains_only_other_with_duplicates() {
    float[] actual = { 1f, 2f, 3f, 4f };
    float[] other = { 2f, 1f, 3f, 4f, 1f, 2f, 3f };
    arrays.assertContainsOnly(description, actual, other);
  }

  private Set<Float> containsOnly(Set<Float> actual, float[] other) {
    Set<Float> notExpected = newLinkedHashSet();
  ...
  private Set<Float> asSet(float[] actual) {
    Set<Float> set = newLinkedHashSet();
    for (float e : actual) {
      set.add(e);
    }
    return set;
