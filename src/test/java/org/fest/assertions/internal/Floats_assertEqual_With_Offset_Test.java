...
public class Floats_assertEqual_With_Offset_Test {
    ...
    try {
      ...
      verify(failures).failure(
          ...
          ShouldBeEqualWithinOffset.shouldBeEqual(actual, expected, offset,
              abs(expected.floatValue() - actual.floatValue())));
      return;
    }
  ...
  public void should_fail_if_actual_is_not_identical_to_expected_with_offset_is_zero() {
    Float actual = new Float(8.122f);
    Float expected = new Float(8.123f);
    try {
      floats.assertEqual(description, actual, expected, Offset.offset(0f));
    } catch (AssertionError e) {
      verify(failures).failure(
          description,
          ShouldBeEqualWithinOffset.shouldBeEqual(actual, expected, Offset.offset(0f), abs(expected.floatValue()
              - actual.floatValue())));
      return;
    }
    expectedAssertionErrorNotThrown();
  ...
  public void should_pass_if_actual_is_equal_to_expected_within_offset() {
    floats.assertEqual(description, 8f, 8f, offset);
  }

  @Test
  public void should_pass_if_actual_is_identical_to_expected_with_offset_is_zero() {
    floats.assertEqual(description, new Float(8.122), new Float(8.122), Offset.offset(0f));
  }
}
