...
/**
 * Tests for <code>{@link Iterables#assertAreNotAtLeast(AssertionInfo, Iterable, Matcher, int)}</code> .
 * 
 * @author Nicolas François
 * @author Mikhail Mazursky
 * @author Joel Costigliola
 */
public class Iterables_assertAreNotAtLeast_Test extends IterablesWithConditionsBaseTest {

  @Test
  public void should_pass_if_not_satisfies_at_least_times_condition() {
    actual = newArrayList("Yoda", "Solo", "Leia");
    iterables.assertAreNotAtLeast(someInfo(), actual, 2, jedi);
    verify(conditions).assertIsNotNull(jedi);
  }

  @Test
  public void should_pass_if_all_not_satisfies_condition_() {
    actual = newArrayList("Leia", "Chewbacca", "Solo");
    iterables.assertAreNotAtLeast(someInfo(), actual, 2, jedi);
    verify(conditions).assertIsNotNull(jedi);
  }
  ...
  public void should_throw_error_if_condition_is_null() {
    ...
    iterables.assertAreNotAtLeast(someInfo(), actual, 2, null);
